...
@section('title', 'FAQ Activity Logs')

@section('content')
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center">
            <h1 class="text-2xl font-semibold text-gray-900">FAQ Activity Logs</h1>
            <a href="{{ route('admin.faqs.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-2 px-4 rounded text-sm flex items-center">
        ...
        <div class="mt-6 bg-white shadow rounded-lg">
            ...
            <div class="p-4 border-b border-gray-200">
                ...
                <form action="{{ route('admin.faqs.activity') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500 max-w-sm truncate">
                                    @if($log->action !== 'deleted' && $log->getEntity())
                                        {{ $log->getEntity()->question ?? 'Unknown' }}
                                    @elseif($log->action === 'deleted' && $log->old_values && isset($log->old_values['question']))
                                        {{ $log->old_values['question'] }}
                                    @else
                                        Unknown
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-500">
                                    @if($log->action !== 'deleted' && $log->getEntity())
                                        {{ $log->getEntity()->category ?? 'None' }}
                                    @elseif($log->action === 'deleted' && $log->old_values && isset($log->old_values['category']))
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                                    No activity logs found for FAQs.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
