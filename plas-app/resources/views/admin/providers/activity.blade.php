...
@section('title', 'Healthcare Provider Activity Logs')

@section('content')
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center">
            <h1 class="text-2xl font-semibold text-gray-900">Healthcare Provider Activity Logs</h1>
            <a href="{{ route('admin.providers.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-2 px-4 rounded text-sm flex items-center">
        ...
        <div class="mt-6 bg-white shadow rounded-lg">
            ...
            <div class="p-4 border-b border-gray-200">
                ...
                <form action="{{ route('admin.providers.activity') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                    ...
                    <div>
                        <label for="type" class="block text-xs font-medium text-gray-700 mb-1">Provider Type</label>
                        <select name="type" id="type" class="w-full rounded-md border-gray-300 shadow-sm focus:border-[#74BA03] focus:ring focus:ring-[#74BA03] focus:ring-opacity-50 text-sm">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($logs as $log)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500">
                                    @if($log->action !== 'deleted' && $log->getEntity())
                                        {{ $log->getEntity()->type ?? 'None' }}
                                    @elseif($log->action === 'deleted' && $log->old_values && isset($log->old_values['type']))
                                        {{ $log->old_values['type'] }}
                                    @else
                                        None
                                    @endif
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500">
                                    @if($log->action !== 'deleted' && $log->getEntity())
                                        {{ $log->getEntity()->city ?? 'None' }}
                                    @elseif($log->action === 'deleted' && $log->old_values && isset($log->old_values['city']))
                                        {{ $log->old_values['city'] }}
                                    @else
                                        None
                                    @endif
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                                    No activity logs found for healthcare providers.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
